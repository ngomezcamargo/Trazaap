...
import { validarContraSchemaOficialEpcis } from './epcis.schema-validator.js';
...
export async function consultarEpcisPorLote(lote, actor) {
  ...
  try {
    ...
    const documento = validarContraSchemaOficialEpcis(construirDocumentoConsultaEpcis(eventos));
...
export async function capturarDocumentoEpcis(documento, actor) {
  habilitado();
  try {
    validarContraSchemaOficialEpcis(documento);
