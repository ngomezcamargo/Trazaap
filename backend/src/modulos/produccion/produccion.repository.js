...
export async function crearRegistroManufactura(data) {
  const { rows } = await poolPostgres.query(
...
export async function actualizarEstadoManufacturaProducto(productoOrdenId, estado) {
  const { rows } = await poolPostgres.query(
...
export async function contarProductosPendientesManufactura(ordenId) {
  const { rows } = await poolPostgres.query(
...
export async function descontarInventarioMateria(data) {
  ...
  const movimientoRes = await poolPostgres.query(
