...
CREATE TABLE IF NOT EXISTS productos_fabricados (
  ...
  prefijo_lote VARCHAR(5) NOT NULL,
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT productos_fabricados_prefijo_lote_key UNIQUE (prefijo_lote),
  CONSTRAINT productos_fabricados_prefijo_lote_check CHECK (prefijo_lote ~ '^[A-Z0-9]{2,5}$'),
  CONSTRAINT productos_fabricados_bagel_prefijo_check CHECK (lower(btrim(nombre)) NOT LIKE 'bagel%' OR prefijo_lote = 'BG')
);
