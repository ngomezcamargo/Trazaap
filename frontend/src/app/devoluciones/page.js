...
export default function DevolucionesPage() {
  ...
  const post = form.tipo_caso === 'devolucion_post_despacho';
  const resolver = async (caso) => { const accion = window.prompt('Accion: retiro, reproceso o destruccion'); if (!['retiro','reproceso','destruccion'].includes(accion)) return; const responsable = window.prompt('ID del operario responsable'); if (!responsable) return; try { await devolucionesServicio.resolver(caso.id_caso, { accion, responsable: Number(responsable), fecha_decision: new Date().toISOString(), observaciones: '' }); setMensaje('Decision registrada; inventario sin cambios automáticos.'); await cargar(); } catch (e) { setError(e.message); } };
  return <GuardiaSesion><GuardiaRol permitido={[ROLES.GERENTE]}><ContenedorApp titulo="Devoluciones y no conformidades" subtitulo="RF16: distingue rechazo previo y devolución posterior al despacho.">
    ...
    <section className="tarjeta"><h3>Casos registrados</h3><div className="tabla-contenedor"><table className="tabla"><thead><tr><th>Tipo</th><th>Lote</th><th>Cliente/despacho</th><th>Cantidad</th><th>Accion</th><th>Impacto inventario</th><th>Responsable</th><th>Gestion</th></tr></thead><tbody>{casos.map(x=><tr key={x.id_caso}><td>{x.tipo_caso}</td><td>{x.lote}</td><td>{x.cliente || '-'}<small className="tabla-subtexto">{x.codigo_despacho || '-'}</small></td><td>{x.cantidad}</td><td>{x.accion}</td><td>{x.impacto_inventario}</td><td>{x.responsable_email}</td><td>{x.accion==='pendiente_decision'?<button className="boton secundario" onClick={()=>resolver(x)}>Resolver</button>:'-'}</td></tr>)}</tbody></table></div></section>
