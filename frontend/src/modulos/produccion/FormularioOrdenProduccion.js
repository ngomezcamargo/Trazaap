...
function etiquetaTab(key) {
  if (key === TABS.ORDENES) return 'Ordenes de produccion';
  if (key === TABS.NUEVA) return 'Nueva orden';
...
function compararManufactura(producto, form) {
  ...
  const filas = [
    {
      ...
      esperado: `${producto.tiempo_fermentacion_minutos || 0} min / ${producto.temperatura_fermentacion_c || 0} C`,
      real: `${form.tiempo_real_fermentacion_minutos || 0} min / ${form.temperatura_real_fermentacion_c || 0} C`,
      desviado:
        Number(form.tiempo_real_fermentacion_minutos || 0) !== Number(producto.tiempo_fermentacion_minutos || 0) ||
        Number(form.temperatura_real_fermentacion_c || 0) !== Number(producto.temperatura_fermentacion_c || 0)
    },
    {
      variable: 'Horneado',
      esperado: `${producto.tiempo_horneado_minutos || 0} min / ${producto.temperatura_horneado_c || 0} C`,
      real: `${form.tiempo_real_horneado_minutos || 0} min / ${form.temperatura_real_horneado_c || 0} C`,
      desviado:
        Number(form.tiempo_real_horneado_minutos || 0) !== Number(producto.tiempo_horneado_minutos || 0) ||
        Number(form.temperatura_real_horneado_c || 0) !== Number(producto.temperatura_horneado_c || 0)
    }
  ];
  ...
  if (producto.requiere_inmersion) {
    filas.push({
      variable: 'Inmersion',
      esperado: `${producto.tiempo_inmersion_minutos || 0} min / ${producto.temperatura_inmersion_c || 0} C`,
      real: `${form.tiempo_real_inmersion_minutos || 0} min / ${form.temperatura_real_inmersion_c || 0} C`,
      desviado:
        Number(form.tiempo_real_inmersion_minutos || 0) !== Number(producto.tiempo_inmersion_minutos || 0) ||
        Number(form.temperatura_real_inmersion_c || 0) !== Number(producto.temperatura_inmersion_c || 0)
    });
  }
...
export function FormularioOrdenProduccion() {
  ...
  const [ordenesManufactura, setOrdenesManufactura] = useState([]);
  ...
  const recargar = async () => {
    ...
    const ops = opsRes.status === 'fulfilled' ? opsRes.value : [];
    setOrdenes(ops);
    setOrdenesManufactura(opsManufacturaRes.status === 'fulfilled' ? opsManufacturaRes.value : []);
    setRecepciones(recsRes.status === 'fulfilled' ? recsRes.value : []);
    setMateriasPrimas(matsRes.status === 'fulfilled' ? matsRes.value.filter((m) => m.is_active) : []);
    ...
    if (fallos.length) setError(fallos[0].reason?.message || 'No fue posible cargar todos los datos de produccion');
  };
  ...
  const ordenesFiltradas = useMemo(() => {
    ...
    return ordenes.filter((o) => {
      ...
      const pasaTexto = !f ? true : `${o.codigo_orden} ${o.fecha_produccion}`.toLowerCase().includes(f);
    ...
    <div className="tarjeta">
      ...
      {tab === TABS.NUEVA && esAdministrador && (
        <>
          ...
          try {
            ...
            if (payloadProductos.some((p) => !p.producto_id || !p.variante_id || !p.cantidad_programada)) {
              throw new Error('Completa producto, variante y cantidad programada antes de guardar la orden.');
            }
          ...
          <div className="grid grid-2">
            ...
            <div className="campo"><label>Estado inicial</label><select value={formOrden.estado} onChange={(e) => setFormOrden({ ...formOrden, estado: e.target.value })}><option value="pendiente">pendiente</option><option value="en_proceso">en_proceso</option><option value="cancelada">cancelada</option></select></div>
          </div>
      ...
      {tab === TABS.MANUFACTURA && !esGerente && (
        <>
          <div className="grid grid-2" style={{ marginBottom: 12 }}>
            <div className="campo">
              <label>Orden pendiente o en proceso</label>
              <select
                value={ordenActivaId}
                onChange={async (e) => {
                  setOrdenActivaId(e.target.value);
                  setManufacturaSeleccionada(null);
                  setContextoManufactura(null);
                  setFormManufactura(nuevaManufactura());
                  if (e.target.value) setDetalle(await produccionServicio.obtenerOrden(Number(e.target.value)));
                }}
              >
                <option value="">Selecciona orden</option>
                {ordenesManufactura.map((o) => (
                  <option key={o.id} value={o.id}>{o.codigo_orden} - {String(o.fecha_produccion).slice(0, 10)} - {o.estado}</option>
                ))}
              </select>
            </div>
          </div>
            ...
            <>
              <div className="grid grid-2">
                <div className="tarjeta">
                  ...
                  <p><strong>Numero:</strong> {detalle.orden.codigo_orden}</p>
                  <p><strong>Fecha:</strong> {String(detalle.orden.fecha_produccion).slice(0, 10)}</p>
                  <p><strong>Estado:</strong> <span className={`estado ${detalle.orden.estado}`}>{detalle.orden.estado}</span></p>
                </div>
                <div className="tarjeta">
                  <h4>Productos programados</h4>
                  <p><strong>Total:</strong> {detalle.productos.length}</p>
                  <p><strong>Pendientes:</strong> {detalle.productos.filter((p) => !['registrado', 'con_observaciones'].includes(p.estado_manufactura)).length}</p>
                </div>
              </div>
                ...
                <tbody>
                  {detalle.productos.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td><span className={`estado ${p.estado_manufactura}`}>{p.estado_manufactura}</span></td>
            ...
            <form onSubmit={guardarManufactura}>
              <div className="grid grid-2">
                <div>
                  ...
                  <p><strong>Cantidad programada:</strong> {contextoManufactura.producto.cantidad_programada}</p>
                  <p><strong>Fermentacion esperada:</strong> {contextoManufactura.producto.tiempo_fermentacion_minutos} min / {contextoManufactura.producto.temperatura_fermentacion_c} C</p>
                  <p><strong>Horneado esperado:</strong> {contextoManufactura.producto.tiempo_horneado_minutos} min / {contextoManufactura.producto.temperatura_horneado_c} C</p>
                  <p><strong>Inmersion:</strong> {contextoManufactura.producto.requiere_inmersion ? `${contextoManufactura.producto.tiempo_inmersion_minutos} min / ${contextoManufactura.producto.temperatura_inmersion_c} C` : 'No aplica'}</p>
                </div>
                <div>
                  <h4>Materias primas asociadas</h4>
              ...
              <div className="grid grid-3">
                <div className="campo">
                  ...
                  <select
                    value={formManufactura.responsable_usuario_id}
                    onChange={(e) => setFormManufactura({ ...formManufactura, responsable_usuario_id: e.target.value })}
                    required
                  >
                ...
                <div className="campo"><label>Temperatura real horneado (C)</label><input type="number" value={formManufactura.temperatura_real_horneado_c} onChange={(e) => setFormManufactura({ ...formManufactura, temperatura_real_horneado_c: e.target.value })} required /></div>
                {contextoManufactura.producto.requiere_inmersion && <div className="campo"><label>Tiempo real inmersion (min)</label><input type="number" min="0" value={formManufactura.tiempo_real_inmersion_minutos} onChange={(e) => setFormManufactura({ ...formManufactura, tiempo_real_inmersion_minutos: e.target.value })} required /></div>}
                {contextoManufactura.producto.requiere_inmersion && <div className="campo"><label>Temperatura real inmersion (C)</label><input type="number" value={formManufactura.temperatura_real_inmersion_c} onChange={(e) => setFormManufactura({ ...formManufactura, temperatura_real_inmersion_c: e.target.value })} required /></div>}
              </div>
              <div className="campo" style={{ marginTop: 8 }}><label>Observaciones</label><textarea value={formManufactura.observaciones} onChange={(e) => setFormManufactura({ ...formManufactura, observaciones: e.target.value })} /></div>

              <h4>Comparacion estandar vs real</h4>
              <table className="tabla">
        ...
        <>
          ...
          {puedeGestionarProductos && mostrarFormularioProducto && <div className="modal-fondo" onClick={cerrarFormularioProducto}><div className="modal" onClick={(e) => e.stopPropagation()}><div className="modal-encabezado-form"><button className="boton secundario modal-cancelar" type="button" onClick={cerrarFormularioProducto}>Cancelar</button><h3>{formProducto.id ? 'Actualizar producto' : 'Agregar producto'}</h3></div>{mensajesModal}<form onSubmit={async (e) => {
            ...
            <div className="grid grid-3">
              ...
              <div className="campo"><label>Prefijo interno del producto</label><input value={formProducto.nombre.trim().toLowerCase().startsWith('bagel') ? 'BG' : formProducto.prefijo_lote} onChange={(e) => setFormProducto({ ...formProducto, prefijo_lote: e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5) })} readOnly={formProducto.nombre.trim().toLowerCase().startsWith('bagel')} minLength="2" maxLength="5" pattern="[A-Z0-9]{2,5}" required /><small>Identificador interno del producto. El lote final se genera exclusivamente en backend con fábrica, fabricación, vencimiento y consecutivo.</small></div>
